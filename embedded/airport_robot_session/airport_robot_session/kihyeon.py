...
def start_background_nodes():
    ...
    nodes = [
        ...
        ["ros2", "launch", "ydlidar_ros2_driver", "ydlidar_launch.py"],  # 필요시 활성화
...
class LineTraceBridge(Node):
    ...
    def start_line_trace(self, timeout: float = 60.0, target_color: str = None) -> bool:
        """라인 트레이싱 시작 및 완료 대기
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        cmd = ["python3", os.path.join(script_dir, "line_follower_node.py")]

        if target_color:
            cmd.extend(["--target-color", target_color])
            self.get_logger().info(f'[실행] line_follower_node.py --target-color {target_color}')
        else:
            self.get_logger().info('[실행] line_follower_node.py (색상 없음)')
...
class RobotSimulator:
    ...
    def _simulate_dispatch(self, mission_id, target_color):
        ...
        print(f"[이동] 라인 트레이싱 시작... (목표 색상: {target_color})")

        if self.line_trace_bridge:
            success = self.line_trace_bridge.start_line_trace(timeout=60.0, target_color=target_color)
            if not success:
                print("[실패] 라인 트레이싱 실패 또는 타임아웃")
                self.state = "IDLE"
        ...
        self.door_locked = True
        print(f"[상태] MOVING_TO_DEST → AT_DESTINATION (라인 트레이싱 완료)")
        print("[응답] 도착 알림 전송")
    ...
    def _simulate_return(self, mission_id):
        ...
        if self.line_trace_bridge:
            success = self.line_trace_bridge.start_line_trace(timeout=60.0)
            if not success:
