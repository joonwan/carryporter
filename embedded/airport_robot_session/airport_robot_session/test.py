...
def start_background_nodes():
    """motor_control, odometry, precise_motion 노드를 백그라운드로 실행"""
    nodes = [
...
class MotionBridge(Node):
    """ROS2 모션 명령 브릿지 - /motion_command 발행, /motion_status 구독"""

    def __init__(self):
        super().__init__('test_motion_bridge')
        self.motion_cmd_pub = self.create_publisher(String, '/motion_command', 10)
        self.motion_status_sub = self.create_subscription(
            String, '/motion_status', self._status_callback, 10
        )
        self.motion_completed = threading.Event()
        self.last_status = ""
        self.get_logger().info('MotionBridge 노드 시작')
    ...
    def send_motion(self, action: str, value: float, timeout: float = 30.0) -> bool:
        """모션 명령 전송 및 완료 대기"""
        ...
        if not self.motion_completed.wait(timeout=timeout):
            self.get_logger().error('[타임아웃] 이동 시간 초과')
            return False
...
class RobotSimulator:
    def __init__(self, mac=None, broker_host=BROKER_HOST, broker_port=BROKER_PORT,
        ...
        # 이동 거리 (m)
        self.move_distance = 0.5  # 50cm 전진

        # ROS2 모션 브릿지 (실제 로봇 이동용)
        self.motion_bridge = None  # 외부에서 설정
    ...
    def _handle_command(self, action, payload):
        """명령 처리 및 자동 응답 (robot_mqtt_node.py 상태 머신 적용)"""
        ...
        if action == "dispatch":
            print(f"[처리] 이동 명령 (직진) - missionId: {mission_id}")
            ...
            print(f"[상태] IDLE → MOVING_TO_DEST (직진)")

            # 비동기 이동 시뮬레이션
            thread = threading.Thread(target=self._simulate_dispatch, args=(mission_id,))
            thread.daemon = True
            thread.start()
    ...
    def _simulate_dispatch(self, mission_id):
        """실제 로봇 전진 이동 (별도 스레드)"""
        print(f"[이동] forward {self.move_distance}m 전진 시작...")

        if self.motion_bridge:
            success = self.motion_bridge.send_motion("forward", self.move_distance)
            if not success:
                print("[실패] 전진 이동 실패")
                self.state = "IDLE"
                self.send_error("ERR_MOTION", "Forward motion failed")
                return
        else:
            print("[경고] MotionBridge 없음 - 3초 시뮬레이션")
            time.sleep(3)

        self.current_location = "destination"
        self.state = "AT_DESTINATION"
        self.door_locked = True
        print(f"[상태] MOVING_TO_DEST → AT_DESTINATION ({self.move_distance}m 전진 완료)")
        print("[응답] 도착 알림 전송")
        self.send_arrived(mission_id)

    def _simulate_return(self, mission_id):
        """실제 로봇 후진 복귀 (별도 스레드)"""
        print(f"[복귀] backward {self.move_distance}m 후진 시작...")

        if self.motion_bridge:
            success = self.motion_bridge.send_motion("backward", self.move_distance)
            if not success:
                print("[실패] 후진 복귀 실패")
                self.state = "AT_DESTINATION"
                self.send_error("ERR_MOTION", "Backward motion failed")
                return
        else:
            print("[경고] MotionBridge 없음 - 3초 시뮬레이션")
            time.sleep(3)
        ...
        self.current_location = "start"
        self.state = "IDLE"
        print(f"[상태] RETURNING → IDLE ({self.move_distance}m 후진 완료, 시작점 복귀)")
        print("[응답] 복귀 완료 전송")
...
# ============== ROS2 + MQTT 초기화 ==============
def init_ros2_motion():
    """ROS2 백그라운드 노드 실행 + MotionBridge 생성"""
    # 1. 백그라운드 노드 자동 실행
    ...
    spin_thread = threading.Thread(target=rclpy.spin, args=(motion_bridge,), daemon=True)
    spin_thread.start()

    # 4. 노드 안정화 대기
    time.sleep(1)
    print("[ROS2] MotionBridge 준비 완료\n")

    return motion_bridge


def shutdown_ros2(motion_bridge):
    """ROS2 정리"""
    try:
        motion_bridge.destroy_node()
        rclpy.shutdown()
    except:
        pass
...
def interactive_test(use_remote=False):
    """대화형 테스트 모드 - 실행 시 자동 등록 + 실제 로봇 이동"""
    # ROS2 모션 초기화
    motion_bridge = init_ros2_motion()
    ...
    # MotionBridge 연결
    robot.motion_bridge = motion_bridge
    ...
    print("\n" + "="*50)
    print("로봇 MQTT + ROS2 모션 테스트")
    print(f"등록 상태: {'완료' if robot.registered else '실패'}")
    print(f"이동 거리: {robot.move_distance}m")
    print("="*50)
    print("명령어:")
    ...
    try:
        ...
        robot.stop()
        shutdown_ros2(motion_bridge)


# ============== 자동 실행 모드 ==============
def auto_run(use_remote=False):
    """자동 실행 - 등록 후 대기 + 실제 로봇 이동"""
    # ROS2 모션 초기화
    motion_bridge = init_ros2_motion()
...
if __name__ == "__main__":
    ...
    print("="*50)
    print("사용법:")
    print("  python test.py          - 로컬 브로커 (대화형)")
    print("  python test.py remote   - 원격 브로커 (대화형)")
    print("  python test.py auto     - 로컬 브로커 (자동)")
    print("  python test.py remote auto - 원격 브로커 (자동)")
    print("="*50 + "\n")
